...
function cmp_render_manual_import_page() {
    ...
    if ( isset( $_POST['cmp_run_import'] ) && check_admin_referer( 'cmp_import_action', 'cmp_import_nonce' ) ) {
        ...
        if (empty($email) || empty($plan_name) || $remaining_days <= 0) {
            ...
            if (!$user) {
                // Auto-generate a secure random password if they don't exist
            ...
            if (!isset($message) || empty($message)) {
                // 2. Save Logistics to User Meta (The Bulletproof Fallback mechanism)
                wp_update_user(array('ID' => $user_id, 'first_name' => $first_name, 'last_name' => $last_name));
                update_user_meta($user_id, 'billing_phone',     $phone);
                ...
                $inserted = $wpdb->insert($table_subs, array(
                    'user_id'            => $user_id,
                    'wc_order_id'        => 0, // "0" acts as the flag for FOH that this was a manual import
                    'plan_name'          => $plan_name . ' (Legacy Import)',
    ...
    <div class="wrap">
        ...
        <div style="background: #fff; padding: 20px 30px; border: 1px solid #ccd0d4; border-radius: 4px; max-width: 800px; box-shadow: 0 1px 1px rgba(0,0,0,.04);">
            ...
            <p style="color: #666; margin-bottom: 25px;">Use this tool to add customers who paid offline or are halfway through an old plan. If the email address doesn't exist in the system, a new account will be auto-created (the customer can use the "Forgot Password" link on the portal to set their own password).</p>
                ...
                <div style="display: flex; gap: 20px; margin-bottom: 25px;">
                    <div style="flex: 2;">
                        ...
                        <select name="plan_name" style="width: 100%; padding: 6px;">
                            <option value="1 Meal Plan">1 Meal Plan</option>
                            <option value="2 Meal Plan">2 Meal Plan</option>
                            <option value="3 Meal Plan">3 Meal Plan</option>
                            <option value="Juice Cleanse">Juice Cleanse</option>
                        </select>
                    ...
                    <div style="flex: 1;">
                        ...
                        <p class="description">How many days left?</p>
                    </div>
                </div>
